...
const { prefetchIssue, setIssueCacheData } = useIssue( issue.value.number, { autoload: false} ); // La autocarga esta desactivada
...
<template>
    ...
    <q-card @mouseenter="setIssueCacheData( issue )" class="text-black col-12 q-mb-md" flat bordered>
        ...
        <q-item-section class="q-pa-xs q-gutter-xs">
            <div>
                <q-chip v-for="label of issue.labels" :key="label.id"
                    :style="{ color: `#${label.color}`, backgroundColor:'#A04000'}"
                    outline
                > 
                    {{ label.name }}
                </q-chip>
